    ...
    <div class="col-span-5 p-4 bg-orange-50 rounded-sm drop-shadow-xl h-full outline outline-offset-1 outline-1">
        <div class="grid grid-cols-1 p-4 divide-y-2 divide-gray-200 divide-solid">
            ...
            <form class="" @submit.prevent="addStats">
                <div class="mt-2">
                    <input type="text" class="rounded-sm p-2" placeholder="Male Population"  v-model="male" />
                </div>
                <div class="mt-2">
                    <input type="text" class="rounded-sm p-2" placeholder="Female Population" v-model="female" />
                </div>
                <div class="mt-2">
                    <select class="form-select p-2" v-model="selectedDemography">
                        <option disabled>Select a Demography</option>
                        <option :value="value" v-for="(value, key) in demography" :key="key">{{key}}</option>
                    </select>
                </div>
                <div class="mt-2" v-if="selectedDemography === 'children'">
                    <select class="form-select p-2" v-model="selectedAgeRange">
                        ...
                        <option :value="value" v-for="(value, key) in ageRange" :key="key">{{key}}</option>
                    </select>
                </div>
                <div class="mt-2">
                    <select class="form-select p-2" v-model="serviceType">
                        <option disabled>Select a Service Type</option>
                        <option :value="value" v-for="(value, key) in churchServices" :key="key">{{key}}</option>
                    </select>
                </div>
                <button class="bg-gray-800 hover:bg-gray-600 rounded-sm p-2 text-white mt-2">Submit</button>      
            </form>
        </div>
    </div>
</template>
